            ...
            if mode == "strict":
                ...
                if len(integer_data) >= 3 and result_speaking_mode and result_speaking_mode.group() == integer_data:
                    ...
                    if decimal_data is not None:
                        ...
                        return sign, integer_data, decimal_data, False
                # 新增：处理"一千五十"这类数字（在万以下且没有百和零的情况下）
            if not any(unit in check_data for unit in ['零']):
            # 检查是否是"X千Y十"的格式
                patterns = [
                    # 处理"X千Y十"格式（如：一千五十）
                    (r'^[一二三四五六七八九]千[一二三四五六七八九]十[一二三四五六七八九]?$', '千', '十'),
                    # 处理"X万Y百"格式（如：五万五百）
                    (r'^[一二三四五六七八九]万[一二三四五六七八九]百[一二三四五六七八九]?$', '万', '百'),
                    # 处理"X万Y十"格式（如：五万五十）
                    (r'^[一二三四五六七八九]万[一二三四五六七八九]十[一二三四五六七八九]?$', '万', '十'),
                    # 处理"X百Y十"格式（如：一百五十，这个可能不需要，因为已经是标准形式）
                    (r'^[一二三四五六七八九]百[一二三四五六七八九]十[一二三四五六七八九]?$', '百', '十'),
                    # 处理"X千Y百"格式（如：一千五百，这个可能不需要，因为已经是标准形式）
                    (r'^[一二三四五六七八九]千[一二三四五六七八九]百[一二三四五六七八九]?$', '千', '百')
                ]
            for pattern, high_unit, low_unit in patterns:
                if re.match(pattern, check_data):
                # 在两个单位之间插入"零"
                    check_data = check_data.replace(high_unit, f'{high_unit}零')
            return sign, integer_data, decimal_data, False
